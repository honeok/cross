...
# https://github.com/spiritLHLS/ecs/blob/main/archive/geekbench5.sh
#
...
benchInit() {
    ...
    case "$(uname -m)" in
        x86_64)
            ...
            Geekbench_downUrl='https://cdn.geekbench.com/Geekbench-5.5.1-Linux.tar.gz'
            GeekbenchVer=5
        ;;
        *i?86*)
            # host is running a 32-bit kernel
            Geekbench_downUrl='https://cdn.geekbench.com/Geekbench-4.4.4-Linux.tar.gz'
            GeekbenchVer=4
        ;;
        *aarch* | *arm*)
            ...
            Geekbench_downUrl='https://cdn.geekbench.com/Geekbench-5.5.1-LinuxARMPreview.tar.gz'
            GeekbenchVer=5
    ...
    if  [ ! -e "$temp_Dir/speedtest-cli/speedtest" ]; then
        ...
        mkdir -p "$temp_Dir/speedtest-cli" && tar -zxf "$temp_Dir/speedtest.tgz" -C "$temp_Dir/speedtest-cli" 2>/dev/null && chmod +x "$temp_Dir/speedtest-cli/speedtest"
    fi
    ...
    if [[ "$GeekbenchTest" == "Y" ]]; then
        ...
        curl -fskL -o "$temp_Dir/Geekbench-Linux.tar.gz" "$Geekbench_downUrl"
        mkdir -p "$temp_Dir/geekbench" && tar -zxf "$temp_Dir/Geekbench-Linux.tar.gz" --strip-components=1 -C "$temp_Dir/geekbench" 2>/dev/null
